...
class poseRecgNet(nn.Module):
    def __init__(self, name_num, pose_num):
        super(poseRecgNet, self).__init__()
        self.conv1 = nn.Conv2d(1, 8, kernel_size=7)
        self.conv2 = nn.Conv2d(8, 32, kernel_size=5)
        self.conv2_drop = nn.Dropout2d()
        self.fc1 = nn.Linear(512, 128)
...
def train_name(epoch, device):
    running_loss = 0.0
    total_loss = 0.0
    for batch_idx, data in enumerate(train_loader, 0):
        img_path, inputs, name_labels, pose_labels = data
        inputs, name_labels = inputs.to(device), name_labels.to(device)
        optimizer.zero_grad()
        name_outputs = model(inputs)
        loss = criterion(name_outputs, name_labels)
        loss.backward()
        optimizer.step()
        # 用item，防止构建计算图
        running_loss += loss.item()
...
def test_name():
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            name_correct += (name_predicted == name_labels).sum().item()
    print('name Accuracy on test set: %d %%' % (100 * name_correct / total))
    return (name_correct / total)


# 训练函数
def train_pose(epoch, device):
    running_loss = 0.0
    total_loss = 0.0
    for batch_idx, data in enumerate(train_loader, 0):
        img_path, inputs, name_labels, pose_labels = data
        inputs, pose_labels = inputs.to(device), pose_labels.to(device)
        optimizer.zero_grad()
        pose_outputs = model(inputs)
        loss = criterion(pose_outputs, pose_labels)
        loss.backward()
        optimizer.step()
        # 用item，防止构建计算图
        running_loss += loss.item()
...
def test_pose():
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            # 列是第1个维度，从左至右
            _, pose_predicted = torch.max(pose_outputs.data, dim=1)
            total += pose_outputs.size(0)
            # 张量之间的比较运算
            pose_correct += (pose_predicted == pose_labels).sum().item()
    print('pose Accuracy on test set: %d %%' % (100 * pose_correct / total))
    return (pose_correct / total)
...
if __name__ == '__main__':
    ...
    plt.subplot(3, 1, 1)
    plt.plot(epochs, n_acc_list, linestyle=line_styles[0], color=gray_colors[0], label='mutil-task')
    plt.legend()
    plt.subplot(3, 1, 2)
    plt.plot(epochs, p_acc_list, linestyle=line_styles[0], color=gray_colors[0], label='mutil-task')
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.plot(epochs, loss_list, linestyle=line_styles[0], color=gray_colors[0], label='mutil-task')
    plt.legend()
    ...
    epochs = range(1, epoch_num + 1)
    plt.subplot(3, 1, 1)
    plt.plot(epochs, n_acc_list, linestyle=line_styles[1], color=gray_colors[1], label='face-task')
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.plot(epochs, loss_list, linestyle=line_styles[1], color=gray_colors[1], label='face-task')
    plt.legend()



    model = poseRecgNet(name_num, pose_num)
    model.to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(model.parameters(), lr=learning_rate, momentum=0.5)
    loss_list = []
    ...
    epochs = range(1, epoch_num + 1)
    plt.subplot(3, 1, 2)
    plt.plot(epochs, p_acc_list, linestyle=line_styles[2], color=gray_colors[2], label='pose-task')
    plt.legend()
    plt.subplot(3, 1, 3)
    plt.plot(epochs, loss_list, linestyle=line_styles[2], color=gray_colors[2], label='pose-task')
    plt.legend()
